...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/10/25 15:32:38 by jlange            #+#    #+#             */
/*   Updated: 2019/10/25 17:37:30 by jlange           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (l->pos[1] < l->nb_raw - 1
			&& layout_case_exist(l, l->pos[0], l->pos[1] + 1))
		l->pos[1] += 1;
	else
	{
		l->pos[1] = 0;
...
void	arrow_left_action(t_layout *l, int t_height)
{
	...
	last_x = (l->nb_args % l->nb_col)
		? (l->nb_args % l->nb_col) - 1 : l->nb_col - 1;
	if (l->pos[0] > 0)
		l->pos[0] -= 1;
...
{
	if (l->pos[0] < l->nb_col - 1
			&& layout_case_exist(l, l->pos[0] + 1, l->pos[1]))
		l->pos[0] += 1;
	else
		l->pos[0] = 0;
	print_args(l, t_height);
